...
export class PacienteComponent implements OnInit {
  ...
  constructor(private model: ModelPaciente, private appService: AppService, private errorService: ErrorService, private alertService: AlertService) {
  ...
  methodFromParent(cell) {
    ...
    this.appService.PacienteGet(id)
      .subscribe(
        response => {
          this.model = response;
        },
  ...
  save(myForm: NgForm) {
    if (myForm.form.valid) {
      if (!this.model.Id) {
        this.appService.PacientePost(this.model)
          .subscribe(
            response => {
        ...
        this.appService.PacientePut(this.model.Id, this.model)
          .subscribe(
            response => {
              this.alertService.success();
              this.clearDto();
  ...
  excluir() {
    ...
    if (ids.length > 0) {
      this.appService.PacienteDelete(ids)
        .subscribe(
          response => {
            this.alertService.success();
            this.grid();
